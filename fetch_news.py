...
CUSTOM_NEWS_SOURCES = [
    ...
    ("经济日报-宏观大政方针", "http://www.people.com.cn/rss/finance.xml"),
    ("新华网-财经要闻", "http://www.xinhuanet.com/finance/news_finance.xml"),
    ("新华网-地方联播", "http://www.xinhuanet.com/local/news_province.xml"),
    # 🏙️【板块三：港澳台与内地综合大报风向】
    # 注：明报、星岛日报、香港经济日报无官方RSS，原rsshub.app公共实例已不可用，已移除
    ("深圳新闻网-深圳特区核心商报", "http://www.xinhuanet.com/local/news_province.xml"),
    # 🌏【板块四：海外华文与全球主流大报】
    # 注：联合早报已关闭RSS服务，已移除
...
def fetch_rss_headlines(name, url):
    ...
    try:
        ...
        # 🛡️ 防假阳性：200但无RSS条目（重定向到HTML、RSS已下线等）
        if not feed.entries:
            return [f"因接口时段限制暂未获取到更新 (RSS源返回空内容)"]
